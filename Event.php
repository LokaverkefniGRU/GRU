...
<?php echo "<a href='event_members.php?id=" .$event_id . "'>View members</a>"; ?>

    <h2><?php echo $name; ?></h2>
    <?php 
        echo "<h5>Start date: " . $start_time . "</h5><br>";
        echo "<h5>End date:   " . $end_time . "</h5><br>";
     ?>
    <form class="well col-lg-12" action="postInEvent.php?id=<?php echo $event_id; ?>" method="POST">
    ...
    <?php
        ...
        $sql = "SELECT post.id, user.image, user.fullname, post.likes, post.title, post.user_ID, post.content, post.photo, post.date_time FROM user JOIN post ON user.id = post.user_id WHERE in_event=1 AND event_ID=$event_id ORDER BY date_time DESC";
                        $result = mysqli_query($db, $sql); 
